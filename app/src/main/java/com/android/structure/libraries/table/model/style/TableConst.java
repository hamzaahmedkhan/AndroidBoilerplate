package com.android.structure.libraries.table.model.style;

public class TableConst {

    public final static int NOBORDERLINE = 0;
    public final static int LEFTBORDERLINE = 0x01;
    public final static int TOPBORDERLINE = 0x02;
    public final static int RIGHTBORDERLINE = 0x04;
    public final static int BOTTOMBORDERLINE = 0x08;
//    public final static int INSIDEHBORDERLINE = 0x10;
//    public final static int INSIDEVBORDERLINE = 0x20;
//    public final static int LT2RBBORDERLINE = 0x40;
//    public final static int LB2RTBORDERLINE = 0x80;

    public static final int ALIGNMENT_GENERAL = 0;
    public static final int ALIGNMENT_LEFT = 1;
    public static final int ALIGNMENT_CENTER = 2;
    public static final int ALIGNMENT_RIGHT = 3;
    public static final int ALIGNMENT_FILL = 4;
    public static final int ALIGNMENT_JUSTIFY = 5;

    public static final int VERTICAL_ALIGNMENT_TOP = 0;
    public static final int VERTICAL_ALIGNMENT_CENTRE = 1;
    public static final int VERTICAL_ALIGNMENT_BOTTOM = 2;
    public static final int VERTICAL_ALIGNMENT_JUSTIFY = 3;
}

